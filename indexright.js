...
addEventBtn.addEventListener("click", () => {
  ...
  const eventInfo = `${eventTitle} - ${eventTimeFrom} to ${eventTimeTo}`;

  // Get the label element where you want to display the event
  const eventsLabel = document.querySelector(".events-label");

  // Update the content of the events label
  eventsLabel.textContent = eventInfo;
...
addEventBtn.addEventListener("click", () => {
  ...
  eventTime.textContent = `${eventTimeFrom} - ${eventTimeTo}`;

  // Append event title and time to event element
  eventElement.appendChild(eventTitle);
  eventElement.appendChild(eventTime);

  // Append event element to events container
